...
function getStackLifo(){   
    fs.readFile('./stackStructured.json', 'utf8', function readFileCallback(err, data) {
        if(err){ 
            ...
            stackLifo = JSON.parse(stackLifoSerialized);    
            console.log("StackLifo has been read");        
        };
    });
};

// function insertStructured (req, res) {

//     stackLifo.push('data item 1');
//     stackLifo.push('data item 2');
//     stackLifo.push('data item 3');
//     stackLifo.push('data item 4');
...
async function setStackStructured(item, insert, res) {
    (async () => {
        ...
        var stackLifoSerialized = serialize.serialize(stackLifo);        
        fs.writeFile("./stackStructured.json", stackLifoSerialized, (err) => {
            if (err) {
                console.error(err);
                return;
            };
